...
const Topic = ({ data, allNames }) => {
  ...
  const convertToObjects = (arr: string[]) => {
    ...
    arr.forEach(string => {
      let qu = string.split('\n');
      let questionAnswer = [qu[0].replace(pattern, ''), qu[1].replace(pattern, '')];
  ...
  return (
    <>
      <NextSeo
        ...
        canonical={`https://namegen.io/${data.Niche.replace(/\s/g, '-').toLowerCase() + '-brand-name-generator'}`}
        openGraph={{
          title: data.MetaTitle,
          description: data.MetaDescription,
          url: `https://namegen.io/${data.Niche.replace(/\s/g, '-').toLowerCase() + '-brand-name-generator'}`,
          images: [
            {
