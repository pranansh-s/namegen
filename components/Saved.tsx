			...
			<div
				...
				<div className="text-primary/60 mb-3 md:text-sm text-xs font-overpass justify-between w-full flex">
					...
					<span
						...
						onClick={() => dispatch(savedArraySet([]))}>
						Clear
					</span>
				</div>
				{saved.length == 0 ? (
					...
					<ul className="text-primary flex flex-col md:text-base text-sm font-overpass h-full mt-2">
						{saved.map((item: any, idx: number) => (
							<li
								key={idx}
								className="flex items-center group border-b-2 border-black/20 mb-2 space-x-10">
								...
								<span className="flex items-center">
									...
									<Image
										...
										src={
											item.domain.includes('undelegated') ? checkCircle : cross
										}
										alt="check"
									/>
									Domains
