...
const Testimonials: FC<{ set: any }> = ({ set }) => {
	...
	const addSubscriber = async () => {
		...
		try {
			const res = await axios.post(
				'https://hook.kntz.it/catch/QUl1g9LRjB-kz5879-NvQ59Xk7po',
	...
	return (
		<div
			className="w-screen h-screen overflow-hidden fixed z-[100] bg-black/30"
			onClick={() => set(false)}>
			<div
				...
				<div className="grid grid-cols-3 items-center relative bg-black px-3 py-8 rounded-t-md">
					...
					<Image
						src={pfp}
						alt="1"
						className="w-20 object-cover h-20 rounded-full mx-auto"
					/>
					<p className="text-white col-span-3 p-1 text-lg mt-5">
						Each week I share research backed name ideas oriented content in{' '}
						<strong>my newsletter.</strong> <br /> There are business, startup,
						and product name ideas. <br /> <strong>14k</strong> entrepreneurs
						read it. I&apos;d love you to join.
					</p>
					{error && (
						<span className="text-left text-sm translate-y-5 text-red-500 font-poppinsRegularItalic">
							* {error}
						</span>
					)}
				</div>
				...
				{success ? (
					<div className="p-2 text-center text-green-600 m-2 bg-white">
				...
				<div className="flex-col sm:flex hidden w-full justify-between items-start space-y-3 text-white px-3 pb-3 pt-5 rounded-b-md bg-black">
					...
					<div className="bg-white text-black p-2 w-full">
						...
						<div className="ml-auto w-max px-2">
							-{testimonials[activeIndex].split('-')[1]}
						</div>
					</div>
				</div>
			</div>
