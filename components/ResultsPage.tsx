...
const ResultsPage: FC = () => {
	...
	const searchTerm = useSelector(
		(state: any) => state.startupNameGenerator.searchTerm
	);
	const industryIndex = useSelector(
		(state: any) => state.startupNameGenerator.industry
	);
	const stylesIndex = useSelector(
		(state: any) => state.startupNameGenerator.styles
	);
	const results = useSelector(
		(state: any) => state.startupNameGenerator.results
	);
	const selectedName = useSelector(
		(state: any) => state.startupNameGenerator.selectedName
	);
	const isSavedOn = useSelector(
		(state: any) => state.startupNameGenerator.isSavedOn
	);
		...
		<div className="w-full h-full xl:px-36 lg:px-20 px-6 py-16 text-sm">
			<div
				className={`${
					results.length != 0
						? 'opacity-0'
						: 'opacity-100 pointer-events-none w-screen h-screen top-0 left-0 fixed'
					...
					<div className="flex lg:flex-row flex-col lg:items-center items-start justify-between lg:bg-transparent bg-white rounded-md shadow-sm lg:px-0 px-5 lg:space-x-3 pb-3 space-x-0 pt-36 xl:w-[calc(100%-18rem)] lg:w-[calc(100%-10rem)] w-[calc(100%-3rem)] mx-auto">
						...
						<div className="w-full flex items-center justify-between z-10 px-2 sm:translate-x-2 -translate-x-2">
							<div className="flex flex-col w-full mb-5">
								<label
									htmlFor="Name"
									className="text-xs mr-auto opacity-60 mb-2">
									I want to name
								</label>
								<input
									type="text"
									...
									onChange={(e) =>
										dispatch(searchTermSet(e.target.value.trim()))
									}
							...
							<>
								<div className="flex w-full justify-between items-center mb-5 space-x-3">
									...
									<div className="flex flex-col w-full">
										...
										<Select
											defaultIndex={stylesIndex
												.map((index: number) => styles[index])
												.map((item) => item.name)}
											callback={stylesSet}
											multiple
