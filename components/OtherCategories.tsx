...
const OtherCategories: FC<{ allNames: any }> = ({ allNames }) => {
	...
	useEffect(() => {
		setMaxPage(
			letters.map((alp: string) =>
				allNames.filter((x: string) => x[0].toUpperCase() == alp)
			).length
		);
	}, [maxPage, allNames]);
