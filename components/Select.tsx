    ...
    if (!multiple) {
      ...
      if (index == 0) {
        ...
        if (index === -1) {
          ...
          if (value !== 'Auto') updatedValues = updatedValues.filter(val => val !== 'Auto');
        } else {
          if (values.length === 1) return;
    ...
    return (
      ...
      >
        {multiple && (
          ...
          >
            {selected && <Image alt="O" src={checkMark} className="h-3 p-[2px]" />}
          </span>
        )}
        <span className={`font-overpass ${selected ? 'font-extrabold' : 'font-normal'}`}>{name}</span>
      </div>
    );
  };
