...
import { pageSet, stylesSet } from '../redux/features/startupNameGeneratorSlice';

import { styles } from '../data/options';

const OptionsStyle = () => {
  const page = useSelector((state: any) => state.startupNameGenerator.page);
  const stylesActive = useSelector((state: any) => state.startupNameGenerator.styles);
  ...
  return (
    ...
    >
      {/* Style Chips */}
      <div className={`overflow-hidden mb-10 h-max transition-all duration-300 w-full lg:px-5 px-2`}>
