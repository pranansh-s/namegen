...
import { selectedNameSet } from '../redux/features/startupNameGeneratorSlice';
import { motion } from 'framer-motion';
import axios from 'axios';
import { resetDomain } from '../redux/features/domainSlice';
...
const NameModal = () => {
  const { selectedName } = useSelector((state: any) => state.startupNameGenerator);
  const domains = useSelector((state: any) => state.domain);
  ...
  const countSyllables = (word: string) => {
    ...
    word = word.replace(/^y/, '');
    return 1 - (word.match(/(?=[aeiouy]{1,2})/g).length + 1) / 10;
  };
  ...
  const scoreScale = (name: string) => {
    let score = countSyllables(name);
    if (score < 0.4) return 'Poor';
    else if (score < 0.5) return 'Average';
    else if (score < 0.6) return 'Excellent';
    else if (score < 0.7) return 'Outstanding';
    else return 'Premium';
  };
          ...
          <div className="bg-[#F9F9FA] sm:w-[50rem] w-full sm:h-[38rem] h-full absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 sm:rounded-3xl sm:overflow-y-hidden overflow-y-scroll p-8 space-y-5">
            ...
            <div className="flex sm:flex-row flex-col sm:h-[45%] h-[28rem] space-x-5 -translate-y-5">
              ...
              >
                <h3
                  style={{
                    fontSize: `${
                      selectedName.font == 'OverThere' ? 1 : Math.min(2.3, 80 / (selectedName.name.length / 1.5))
                    }rem`,
              ...
              <div className="py-3 sm:text-3xl text-4xl flex flex-col sm:items-start items-center">
                ...
                <span
                  className={`flex text-[0.8rem] text-secondary w-max ${
                    isSpeaking ? 'cursor-default opacity-60' : 'cursor-pointer opacity-100'
                  }`}
                ...
                <div className="mt-6 flex flex-col text-sm space-y-4 font-poppinsRegular items-center">
                  <div className="relative sm:w-full w-1/2">
                    <Image src={syllables} alt="syllables" className="w-full absolute" />
                    <svg
                      width="100%"
                      height="100%"
                      viewBox="0 0 217 44"
                    ...
                    >
                      <motion.path
                        ...
                        animate={{
                          pathLength: countSyllables(selectedName.name),
                        }}
                        ...
                        stroke={
                          countSyllables(selectedName.name) < 0.4
                            ? '#E84E4E'
                            : countSyllables(selectedName.name) < 0.5
                            ? '#E88F4E'
                            : countSyllables(selectedName.name) < 0.6
                            ? '#E8D84E'
                            : countSyllables(selectedName.name) < 0.7
                            ? '#C0E84E'
                            : '#76E84E'
                        }
                        strokeWidth="15"
                  ...
                  <span className="flex justify-start">
                    {Math.floor((1 - countSyllables(selectedName.name)) * 10)} syllables &nbsp;&nbsp;{' '}
                    <span className="text-secondary">•</span> &nbsp;&nbsp; {scoreScale(selectedName.name)}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div>
              <h5 className="text-secondary font-poppinsSemiBold  flex items-center">
                <Image src={globe} className="w-5 mr-1" alt="globe" /> Domain Availability
              </h5>
              <ul className="grid sm:grid-cols-2 grid-cols-1 gap-3 mt-3">
                {['com', 'net', 'org', 'me', 'xyz', 'co'].map((domain, idx) => (
                  <li key={idx} className="flex items-center justify-center">
                    ...
                    <span
                      className={`font-poppinsSemiBold flex justify-center lg:mt-[0.4rem] mt-[0.25rem] ${
                        domains.loading
              ...
              <span
                ...
                onClick={() => handleLinkClick(link)}
              >
                <span className="underline text-sm">Show More</span> &nbsp; <Image src={linkW} alt="link" />
              </span>
            </div>
            <div className="text-secondary font-poppinsSemiBold flex items-center">
              ...
              <span
                ...
                onClick={() => domains.trademark == true && handleLinkClick(`register-trademark`)}
              >
                {domains.trademark == null ? 'loading' : domains.trademark == true ? 'available' : 'not available'}
                {domains.trademark && <Image src={linkG} alt="link" className="p-1 w-6" />}
              </span>
            </div>
          </div>
          <div
