...
const NameModal = () => {
	...
	function breakIntoSyllables(word: string) {
		const vowels = word.match(/[aeiou]+|[^aeiouy]+/gi);
		...
		for (let i = 0; i < vowels.length; i++) {
			const vowelIndex = word.indexOf(vowels[i], prevIndex);

			let syllable = word.slice(prevIndex, vowelIndex + vowels[i].length);
			syllabified += syllable;

			prevIndex = vowelIndex + vowels[i].length;

			if (i < vowels.length - 1) {
				syllabified += '•';
			}
		}
		syllabified += word.slice(prevIndex);
		return syllabified;
	}

	function sizeOf(word: string) {
		let l = countSyllables(word) / 20;
		if (l < 0.2) return 'Very Small';
		else if (l < 0.4) return 'Small';
		else if (l < 0.6) return 'Moderate';
		else if (l < 0.8) return 'Long';
		else return 'Very Long';
	}

	function countSyllables(word: string) {
		const syllableWord = breakIntoSyllables(word);
		return syllableWord.split('•').length;
	}
							...
							<div className="py-3 sm:text-3xl text-4xl flex flex-col sm:items-start items-center">
								...
								<div className="mt-6 flex flex-col text-sm space-y-4 font-poppinsRegular items-center">
									<div className="relative sm:w-full w-1/2">
										...
										<svg
											...
											<motion.path
												...
												animate={{
													pathLength: countSyllables(selectedName.name) / 20,
												}}
												...
												stroke={
													countSyllables(selectedName.name) / 20 < 0.2
														? '#76E84E'
														: countSyllables(selectedName.name) / 20 < 0.4
														? '#C0E84E'
														: countSyllables(selectedName.name) / 20 < 0.6
														? '#E8D84E'
														: countSyllables(selectedName.name) / 20 < 0.8
														? '#E88F4E'
														: '#E84E4E'
												}
												strokeWidth="15"
									...
									<span className="flex justify-start">
										...
										{sizeOf(selectedName.name)} Length
									</span>
								</div>
							</div>
						</div>
